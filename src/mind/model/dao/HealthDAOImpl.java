...
public class HealthDAOImpl implements HealthDAO {
	...
	/*
	 * @Override public int insertPoint(String memberId, Connection con) throws
	 * SQLException { PreparedStatement ps = null; String sql =
	 * proFile.getProperty("point.insert"); int result = 0;
	 * 
	 * ps = con.prepareStatement(sql); ps.setString(1, memberId); result =
	 * ps.executeUpdate(); return result; }
	 */
	...
	public List<GymDTO> selectAllGym() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				GymDTO gym = new GymDTO(code, name, addr, phoneNum, fileName, gymCapacity, price, comment, weekdayHour,
						weekendHour, avgScore);
	...
	public List<GymDTO> selectGymByKeyword(String keyField, String keyword) throws SQLException {
		...
		String sql = proFile.getProperty("gym.selectByKeyword");// sql = gym.selectByKeyword=SELECT CODE, NAME, ADDR,
																// PHONE_NUM, FILE_NAME, GYM_CAPACITY, PRICE,
																// GYM_COMMENT, WEEKDAY_HOUR, WEEKEND_HOUR, STAR_SCORE
																// FROM GYM WHERE ? LIKE ?
	...
	public GymDTO selectGymByCode(int gymCode) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("gym.selectByCode");// sql = SELECT CODE, NAME, ADDR, PHONE_NUM, FILE_NAME,
																// GYM_CAPACITY, PRICE, GYM_COMMENT, WEEKDAY_HOUR,
																// WEEKEND_HOUR, STAR_SCORE FROM GYM WHERE CODE = ?
		ResultSet rs = null;
		GymDTO gym = null;
		...
		try {
			...
			if (rs.next()) {
				...
				gym = new GymDTO(code, name, addr, phoneNum, fileName, gymCapacity, price, comment, weekdayHour,
						weekendHour, avgScore);
	...
	public int insertReview(ReviewDTO review) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("review.insert"); // INSERT INTO REVIEW (CODE, GYM_CODE, MEMBER_ID, REG_DATE,
															// STAR_SCORE, CONTENT, FILE_NAME)
															// VALUES(REVIEW_CODE_SEQ.NEXTVAL, ?, ?, SYSDATE, ?, ?, ?)
		int result = 0;
	...
	public List<ReviewDTO> selectReviewByGymCode(int gymCode) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("review.selectByGymCode");// SELECT CODE, GYM_CODE, MEMBER_ID, REG_DATE,
																	// STAR_SCORE, CONTENT, FILE_NAME FROM REVIEW WHERE
																	// GYM_CODE = ?
		System.out.println(sql);
		ResultSet rs = null;
	...
	public int updateReview(ReviewDTO review) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("review.update"); // UPDATE REVIEW SET STAR_SCORE = ?, CONTENT = ?, FILE_NAME =
															// ? WHERE CODE = ? AND MEMBER_ID = ?
		int result = 0;
	...
	public int insertUseDetail(UseDetailDTO useDetail) throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("useDetail.insert"); // INSERT INTO USE_DETAIL(CODE, MEMBER_ID, GYM_CODE,
																// PRICE, USE_START_HOUR, STATE) SELECT
																// USE_DETAIL_SEQ.NEXTVAL, MEMBER.ID, GYM.CODE,
																// GYM.PRICE, SYSDATE, 1 FROM MEMBER, GYM WHERE
																// MEMBER.ID = ? AND GYM.CODE = ?
		int result = 0;
...
//			System.out.println("keyworld" + keyword);
	...
	public List<UseDetailDTO> selectUseDetailByGymCodeState(int gymCode, int state) throws SQLException {
		...
		String sql = proFile.getProperty("useDetail.selectByGymCodeState"); // SELECT CODE, MEMBER_ID, GYM_CODE, PRICE,
																			// USE_START_HOUR, STATE FROM USE_DETAIL
																			// WHERE GYM_CODE = ? AND STATE = 1
	...
	public int checkUseDetailState() throws SQLException {
		Connection con = null;
		PreparedStatement ps = null;
		String sql = proFile.getProperty("useDetail.CheckStateUpdate"); // UPDATE USE_DETAIL SET STATE = CASE WHEN
																		// SYSDATE - TO_DATE(USE_START_HOUR) > 1.0 THEN
																		// -1 ELSE 1 END
		int result = 0;
		// System.out.println(sql);
		try {
